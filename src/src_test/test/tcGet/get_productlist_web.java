...
public class get_productlist_web extends base{
    ...
    @Test
    public void getproductlist_web() {
...
//                header("Authorization", prop.getProperty("token_web")).
                when().
                get("/products?cat_slugs=cleanser").
                then().assertThat().statusCode(200).and().contentType(ContentType.JSON).and().
                header("Server", "nginx/1.10.3 (Ubuntu)").log().body().  //log untuk log request
                extract().response();

        String response_productname = res.path("data.name[0]");
